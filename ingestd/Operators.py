...
from ingestd.kafka.SchemaRegistryAbstraction import Schema, JSONSchema
from pathlib import Path
from confluent_kafka import Message
from ingestd.kafka.finwire.utils import find_type
...
class FlatFileConnector(object):
    def __init__(self, **kwargs):
        """
        :param str file_path:
        :param str file_format: "fwf, csv, tsv, bar, xml, txt"
        """
        self.file_path = kwargs['file_path'] or None
        self.file_format = kwargs['file_format'] or None
        self.file_type = self.file_path.split('\\.')[-1] or None
        self.delimiter = kwargs['delimiter'] or None

    def route(self):
        if self.file_format in ["csv", "tsv", "bar", "txt"]:
            operator = DelimiterOperator(self.file_path, self.delimiter)
        elif self.file_format == 'xml':
            operator = XMLOperator(self)
...
class DelimiterOperator(object):
...
class FixedWidthOperator(FlatFileConnector):
    def __init__(self):
        super(FlatFileConnector, self).__init__(**kwargs)

    def create_parser(self, fieldWidths):
